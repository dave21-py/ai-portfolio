...
// <-- UPDATED ICONS: Re-added FaUserAlt for the new "About Me" button
import { FaGithub, FaBriefcase, FaLayerGroup, FaUserFriends, FaBuilding, FaArrowLeft, FaFileAlt, FaUserAlt } from 'react-icons/fa';
...
export default function Home() {
  ...
  const memojiVariants: Variants = {
    ...
    visible: {
      ...
      translateY: [0, -8, 0],
      transition: {
        delay: 0.5, type: "spring", damping: 10, stiffness: 100,
  ...
  return (
    <div className="relative min-h-screen bg-white overflow-hidden">
      ...
      <AnimatePresence>{isChatActive && (<motion.button onClick={handleGoBack} className="absolute top-6 left-6 z-40 bg-white/50 backdrop-blur-md border border-gray-200/80 text-gray-800 px-3 py-2 rounded-lg flex items-center gap-2 hover:scale-105 transition shadow-sm" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}><FaArrowLeft /><span className="hidden sm:inline">Back</span></motion.button>)}</AnimatePresence>
      <a href="https://github.com/dave21-py" target="_blank" rel="noopener noreferrer" className="absolute top-6 right-6 z-40 bg-white/50 backdrop-blur-md border border-gray-200/80 text-gray-800 px-4 py-2 rounded-lg flex items-center gap-2 hover:scale-105 transition shadow-sm"><FaGithub /></a>

      <div className="relative z-30 flex flex-col items-center">
        <AnimatePresence mode="wait">
          {!isChatActive ? (
            <motion.div key="welcome" className="mt-[8vh] mb-8 text-center" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, y: 20 }}>
              <div className="bg-gray-50/50 backdrop-blur-xl border border-gray-200/50 rounded-2xl p-8 shadow-2xl max-w-3xl mx-auto">
                ...
                <motion.h1 variants={containerVariants} className="text-6xl font-bold text-black mt-4 leading-tight">{"Welcome to my AI Portfolio".split(" ").map((word, i) => (<motion.span key={i} variants={textVariants} className="inline-block mr-4">{word}</motion.span>))}</motion.h1>
                <motion.div variants={memojiVariants}><Image src="/memo.png" alt="David's Memoji" width={140} height={140} className="mt-8 mx-auto"/></motion.div>
              </div>
            </motion.div>
          ) : (
            <motion.div key="chat" className="w-full max-w-2xl text-black space-y-6 text-left mt-[15vh]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }}>
              {chatHistory.map((msg, index) => (<div key={index}><p className="text-lg"><strong className={msg.role === 'user' ? 'text-blue-600' : 'text-emerald-600'}>{msg.role === 'user' ? 'You:' : 'AI:'}</strong>{' '}{msg.text}</p></div>))}
      ...
      <motion.div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 w-full max-w-5xl flex flex-col items-center gap-4 z-40" variants={bottomNavVariants} initial="hidden" animate="visible">
        <ChatInput onAction={handleAction} />
        
        <AnimatePresence>
          {!isChatActive && (
            // <-- FINAL NAVIGATION: "About Me" added, "Fun" removed, all in logical order.
            <motion.nav className="flex gap-4" variants={containerVariants} initial="hidden" animate="visible">
              ...
              <motion.div variants={textVariants}><button onClick={() => handleAction('showMe', 'about me')} className="bg-white/60 backdrop-blur-md border border-gray-200/80 rounded-2xl p-3 w-24 h-20 flex flex-col items-center justify-center gap-1 hover:-translate-y-1 transition text-sm shadow-md"><FaUserAlt size={20} className="text-teal-500" /><span className="text-gray-800 font-medium">About Me</span></button></motion.div>
              <motion.div variants={textVariants}><button onClick={() => handleAction('showResume', 'my resume')} className="bg-white/60 backdrop-blur-md border border-gray-200/80 rounded-2xl p-3 w-24 h-20 flex flex-col items-center justify-center gap-1 hover:-translate-y-1 transition text-sm shadow-md"><FaFileAlt size={20} className="text-gray-600" /><span className="text-gray-800 font-medium">Resume</span></button></motion.div>
